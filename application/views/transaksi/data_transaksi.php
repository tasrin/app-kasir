		...
		<div class="row">
			<div class="panel panel-default">
				<div class="panel-body">
					<h5><i class='fa fa-shopping-cart fa-fw'></i> <?php echo $title; ?> <i class='fa fa-angle-right fa-fw'></i> Penjualan</h5>
					<hr />
					<div class="col-sm-3">
						<div class="panel panel-primary">
							...
							<div class="form-horizontal">
								<div class="form-group">
									...
									<div class="col-sm-8">
										<input type='text' name='nomor_nota' class='form-control input-sm' id='nomor_nota' value="<?php echo strtoupper(uniqid()).$this->session->userdata('id_user'); ?>" <?php echo $readonly; ?>>
									</div>
								</div>
								<div class="form-group">
					...
					<div class="panel panel-primary" id='PelangganArea'>
						...
						<div class="panel-body">
							<div class="form-group">
								...
								<a href="<?php echo site_url('transaksi/tambah_pelanggan'); ?>" class='pull-right' id='TambahPelanggan'>Tambah Baru ?</a>
					...
					<table class='table table-bordered' id='TabelTransaksi'>
						<thead>
							<tr >
								...
								<th style='width:100px;'>Jumlah Beli</th>
					...
					<div class='alert alert-info TotalBayar'>
						<button style="margin-top:- 20px;float: left;" id='BarisBaru' class='btn btn-default'><i class='fa fa-plus fa-fw'></i> Baris Baru (F7)</button>
						...
						<div class='col-sm-5'>
							<div class="form-horizontal">
								<div class="form-group">
									...
									<div class="col-sm-6">
										<input style="font-size: 20px;font-weight: bold;font-family: sans-serif;" type='text' name='cash' id='UangCash' class='form-control' onkeypress='return check_int(event)'>
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-6 control-label">Kembali</label>
									<div class="col-sm-6">
										<input style="font-size: 20px;font-weight: bold;font-family: sans-serif;" type='text' id='UangKembali' class='form-control' disabled>
									</div>
								</div>
								<div class='row'>
...
$(document).ready(function(){
	...
	$('#id_pelanggan').change(function(){
		...
		{
			$.ajax({
				url: "<?php echo site_url('transaksi/ajax_pelanggan'); ?>",
				type: "POST",
				cache: false,
...
function BarisBaru()
{
	...
	var Baris = "<tr>";
		Baris += "<td>"+Nomor+"</td>";
		Baris += "<td>";
			Baris += "<input type='text' class='form-control' name='kd_barang[]' id='pencarian_kode' placeholder='Ketik Kode / Nama Barang'>";
		...
		Baris += "<td>";
			...
			Baris += "<span></span>";
		Baris += "</td>";
		Baris += '<td><input type="text" id="jumlah_beli" class="form-control text-center" name="jumlah_beli[]" onkeypress="return check_int(event)" disabled></td>';
		Baris += "<td>";
...
function AutoCompleteGue(Lebar, KataKunci, Indexnya)
{
	...
	$.ajax({
		url: "<?php echo site_url('transaksi/ajax_kode'); ?>",
		type: "POST",
		cache: false,
...
$(document).on('keyup', '#jumlah_beli', function(){
	...
	$.ajax({
		...
		type: "POST",
		cache: false,
		data: "kd_barang="+encodeURI(KodeBarang)+"&stok="+JumlahBeli,
		dataType:'json',
...
function SimpanTransaksi()
{
	...
	$.ajax({
		url: "<?php echo site_url('transaksi/data_transaksi'); ?>",
		type: "POST",
		cache: false,
		data: FormData,
		dataType:'json',
		success: function(data){
			if(data.status == 1)
			{
				...
				$('#ModalContent').html(data.pesan);
				$('#ModalFooter').html("<button type='button' class='btn btn-primary' id='refresh' data-dismiss='modal'>Ok</button>");
				$('#ModalGue').modal('show');

				setTimeout(function(){ 
			   		$('button#refresh').focus();
			    }, 100);
				window.location.href="<?php echo site_url('transaksi/data_transaksi'); ?>";
			}
			else 
			{
				$('.modal-dialog').removeClass('modal-lg');
